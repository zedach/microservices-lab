 ...
 * Open the kafka resource using command `oc edit kafka my-cluster`.
 * Paste the content of the file `https://raw.githubusercontent.com/debezium/microservices-lab/master/vm-setup/kafka-metrics.yaml` right under the `kafka:` line that represents the start of the broker configuration.
Make sure that the resulting text is still valid YAML file.
