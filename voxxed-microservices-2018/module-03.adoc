...
[source,sh]
----
$ oc new-app --name=hikingapp openshift/wildfly-140-centos7:latest~https://github.com/debezium/microservices-lab.git \
...
This happens in the https://github.com/debezium/microservices-lab/blob/master/debezium-swarm-demo/src/main/java/com/example/dbzdemo/ws/WebSocketChangeEventHandler.java[WebSocketChangeEventHandler] class.
All it then needs to do is to push all incoming events via WebSockets to all connected clients.
For that purpose, the https://github.com/debezium/microservices-lab/blob/master/debezium-swarm-demo/src/main/java/com/example/dbzdemo/ws/ChangeEventsWebsocketEndpoint.java[ChangeEventsWebsocketEndpoint] class registers all clients with the event handler upon connection creation.
